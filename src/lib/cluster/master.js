...
function send(msg) {
	...
	//Round robin send the message to one of the cluster workers
	var current = Object.keys(cluster.workers);
	
	//Get the next worker in the round robin
	//TODO check worker status first?
	currentWorker++;

	//Reset the current worker to the start 
	if(currentWorker >= current.length) currentWorker = 0;

	//Send the message to this worker
	cluster.workers[ current[currentWorker] ].send(msg);
}
...
function clusterMaster (config) {
  ...
  env = config.env;
	...
	//Allow an unlimited number of event listeners on the cluster
	// TODO set this to the number of workers??
	//cluster.setMaxListeners(0);

  var masterConf = { exec: path.resolve(config.exec) };
  if (config.silent) masterConf.silent = config.silent;
  ...
  if (config.signals) setupSignals();
...
function forkListener() {
  ...
  cluster.on("fork", function (worker) {
	...
	//Register the worker events
	for(var event in events) 
		if(typeof events[event] === 'function') worker.on(event, events[event]);
